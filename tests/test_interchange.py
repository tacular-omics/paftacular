...
def test_structured_internal_preserves_backbone():
    annotation = p.PafAnnotation(p.InternalFragment(2, 3, nterm_ion_type=p.IonSeries.B, cterm_ion_type=p.IonSeries.X))
...
def test_batch_retains_errors_and_record_indices():
    results = list(p.iter_parse(["y2,b3", "y2,b3!", "p", ""]))
