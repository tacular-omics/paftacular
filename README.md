...
- **Typed** (`py.typed`) and fast: repeated ion and modifier text shares
  immutable components through bounded parser caches.
...
print(ann.get_mass())  # 19.017841466621 (offset only, no sequence context)
...
There are 2 parsing entry points: `parse` (exactly one annotation, raises
`PafParseError` otherwise) and `parse_multi` (comma-separated, always a list).
Every error caused by bad input is a `PaftacularError`, which is a `ValueError`.
...
print(ann.sequence, ann.mz())  # DE 263.087377579721
```
